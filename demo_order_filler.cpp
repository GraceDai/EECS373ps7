...
int main(int argc, char** argv) {
    ...
    cout << "enter 1 to get box pose at Q1: ";
    cin>>ans;
    ...
    int nparts_misplaced1 = misplaced_models_actual_coords_wrt_world.size();
   
    ROS_INFO("found %d misplaced parts", nparts_misplaced1);
    bool go_on1 = false;
    
    for(int i = 0; i < nparts_misplaced1; i++){
    ...
    for(int i = 0; i < part_indices_missing.size(); i++){
        int n_missing_part1 = part_indices_missing[i];
        //model_to_part(desired_models_wrt_world[n_missing_part1], current_part, inventory_msgs::Part::QUALITY_SENSOR_2);
        std::string part_name(desired_models_wrt_world[n_missing_part1].type);
        ...
        //specify place part:
        model_to_part(desired_models_wrt_world[n_missing_part1], place_part, inventory_msgs::Part::QUALITY_SENSOR_2);

        go_on1 = robotBehaviorInterface.evaluate_key_pick_and_place_poses(pick_part, place_part);
        if (!go_on1) {
